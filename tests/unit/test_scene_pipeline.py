...
def test_parametric_lattice_mount_zone_allows_valid_lower_hba_on_tall_requested_tower() -> None:
    registry = AssetRegistry(Path("assets/manifests"))
    requirements = parse_requirements_text(
    ...
    scene = ScenePlanner().build_scene_spec("wf_lattice_90", requirements, tower, antenna, radio)
    scene_report = validate_scene_spec(scene, registry.list_assets())
