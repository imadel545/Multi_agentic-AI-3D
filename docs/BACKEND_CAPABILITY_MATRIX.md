...
| Shared Groq gateway | IMPLEMENTED_LIMITED | `core/llm/transport.py`, `core/llm/profiles.py`, FastAPI lifespan | Pool multi-comptes rétrocompatible, HTTP persistant, sélection least-in-flight/round-robin, concurrence bornée par credential, quarantaine 401, restriction capability 403, cooldown 429 et 413/`rate_limit_exceeded` sans sleep du worker, tentatives transitoires globales bornées, circuit par capability après épuisement et erreurs nettoyées. Les statuts agrégés ne révèlent aucune identité de clé. Les deux comptes et le flux Groq/NVIDIA/Blender ont passé la gate live du 2026-08-11; le timeout reste par tentative et cette preuve n'est pas une garantie permanente. |
