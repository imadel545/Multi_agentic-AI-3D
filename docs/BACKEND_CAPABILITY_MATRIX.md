...
| Document pack multi-fichiers / ZIP | IMPLEMENTED_LIMITED | `core/document_pack/service.py`, `core/document_pack/mapper.py`, `POST /document-packs` | ZIP brut ou multipart direct; synchrone, 256 pièces, 15 Mo/pièce, 200 Mo non compressés et 80 Mo compressés; noms normalisés et doublons refusés; extraction en mémoire. Fondation absente/incompatible partagée par résumé, QA, gate et formulaire de correction. |
...
| Memory | IMPLEMENTED_LIMITED | `core/memory`, `POST /memory/vector/reindex`, collections Qdrant atomiques et versionnées | SQLite reste canonique. La projection vectorielle compacte les designs par signature technique et les erreurs par motif, puis publie atomiquement un index identifié par provider/modèle/profil/dimension; les collections legacy restent conservées. Recall workflow encore principalement SQL; état et URL de reconstruction visibles dans `/studio/summary`. |
