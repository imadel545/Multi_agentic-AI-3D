...
| Shared Groq gateway | IMPLEMENTED_LIMITED | `core/llm/transport.py`, `core/llm/profiles.py`, FastAPI lifespan | Transport HTTP persistant partagé, retries transitoires bornés, `Retry-After`, circuit breaker par capability, erreurs nettoyées et fermeture au lifespan. `openai/gpt-oss-120b` garde le JSON Schema strict. Aucun probe provider live M1 ne qualifie la disponibilité externe. |
