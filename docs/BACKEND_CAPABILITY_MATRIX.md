...
| Frontend product rework | IMPLEMENTED_LIMITED | `apps/frontend`; 157 tests Vitest, typecheck et build passés; smoke connecté courant le 2026-08-10 | Baseline chat-first/3D-first. Le bootstrap charge l'inventaire réel; Composition distingue qualification runtime et preuve professionnelle, masque les previews non éligibles, expose risques/fallbacks en langage utilisateur et marque la stratégie comme planifiée. Le smoke courant restaure un GLB certifié sans erreur console, mais aucun parcours professionnel M1 complet n'existe. |
...
| Generic asset intelligence | IMPLEMENTED_LIMITED | `QualifiedAssetCandidateRetriever`, `AssetDecisionPacket`, décisions de stratégie cognitives | Le core générique reçoit désormais les candidats qualifiés et leurs stratégies autorisées, mais `allowed_strategies=[]` tant que son compilateur ne sait pas exécuter `reuse`/`adapt`/`compose`. Il reste donc procédural à l'exécution et ne doit pas revendiquer une réutilisation générique réelle. |
