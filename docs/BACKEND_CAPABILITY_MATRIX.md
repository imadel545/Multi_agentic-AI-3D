...
| Asset inventory | IMPLEMENTED_LIMITED | `/assets/inventory`, `/assets/{asset_id}/provenance`, `/assets/{asset_id}/previews/{view}`, `core/services/asset_evidence.py`, `core/services/asset_inventory.py` | 13 manifests utilisables par le runtime historique, mais 0/13 passe le gate professionnel M1. Le flag public vient d'une vérification runtime fail-closed des chemins, octets/hashes, GLB, PNG, rapport QA, lignée, dimensions et anchors; la complétude déclarative du manifest ne suffit plus. Le verifier ne qualifie ni droits ni sémantique B-Rep. |
