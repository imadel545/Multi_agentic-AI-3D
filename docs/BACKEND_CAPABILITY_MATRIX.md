...
| Blender generation | IMPLEMENTED_LIMITED | `core/services/blender_runner.py`, `apps/blender_worker`, `trusted_assembly.py`, `component_proofs.py`, `geometry_program_compiler.py` | Réel si Blender trouvé; `AssemblyPlan 1.1` et registry déterministe gouvernent l'exécution. Le worker isolé revalide manifests, assets exacts, builders et opérations avant construction. Build lock `1.2.0` lie bundle, runtime, artefacts et entrées fiables; fallback Blender refusé par défaut. |
...
| Frontend product rework | IMPLEMENTED_LIMITED | `apps/frontend`; 139 tests Vitest, typecheck et build passés sur l'arbre M0; smokes réels historiques enregistrés | Baseline chat-first/3D-first; erreurs et retries indépendants pour GLB/WebGL/assets/QA/RAG/provenance/documents, sans succès obsolète. Le smoke navigateur connecté sur l'arbre final reste la seule gate M0 ouverte. |
