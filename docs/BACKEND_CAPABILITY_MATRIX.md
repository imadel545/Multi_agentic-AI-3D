...
| Design blueprint | IMPLEMENTED_LIMITED | `compose_design_blueprint`, `core/contracts/design_blueprint.py`, `design_blueprint.json`, `assembly_plan.json`, deux rapports de couverture | Intents génériques, contraintes et DAG de spécialistes déterministes routés; scoring multicandidat explicable, connecteurs et preuve `RequirementSpec -> Blueprint -> SceneSpec` liée au certificat 1.1. La diversité de candidats reste faible et aucun superviseur LLM ne crée librement de domaines. |
| Typed out-of-catalog geometry | IMPLEMENTED_LIMITED | `GeometryRequest`, `GeometryProgramPlanner`, `core/contracts/geometry_program.py`, `apps/blender_worker/geometry_program_compiler.py`, `plan_generated_geometry` | GPT-OSS écrit uniquement primitives/courbes/instances/matériaux/transforms typés. Enveloppe maximale validée déterministiquement avec adaptateur uniforme borné pour ce seul dépassement; 1024 nœuds agrégés max; aucun Python LLM. Pas de CSG, jugement sémantique du prompt, placement interprété, BVH ou certification fournisseur. |
| Asset inventory | IMPLEMENTED | `/assets/inventory`, `core/services/asset_inventory.py` | 13 manifests, 12 GLB, 12 profils éligibles, 3 GLB autorisés en import exact, 9 profils paramétriques et 1 référence seule; statut `qualified_mixed_catalog`. |
...
| Blender generation | IMPLEMENTED_LIMITED | `core/services/blender_runner.py`, `apps/blender_worker`, `geometry_program_compiler.py` | Réel si Blender trouvé; builders déterministes et compilateur de données GeometryProgram, factory-startup, staging unique par tentative, copie immuable du bundle worker réellement exécutée et build lock hashé; fallback Blender refusé par défaut côté qualité. |
| Missing asset fallback | IMPLEMENTED | `apps/blender_worker/generate_scene.py` | Tous les manifests tower disposent d'un GLB. Les échecs d'import autorisés et la route câble sans GLB qualifié restent des fallbacks procéduraux explicites. |
...
| Frontend product rework | IMPLEMENTED_LIMITED | `apps/frontend`; 125 tests Vitest, typecheck et build passés le 2026-07-31; smokes réels enregistrés | Baseline compositeur-first/3D-first; contradiction d'entrée bloquée, fallback/transport humanisés, intents et provenance GeometryProgram visibles, fidélité géométrique visible, fondation documentaire corrigeable. Full acceptance exige encore un smoke visuel enregistré de chaque mutation produit. |
