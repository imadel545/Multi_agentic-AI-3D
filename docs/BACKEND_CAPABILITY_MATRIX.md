...
| Memory | IMPLEMENTED_LIMITED | `core/memory`, `POST /memory/vector/reindex`, collections Qdrant atomiques et versionnées | SQLite reste canonique. Publication/invalidation sérialisées, recherche limitée aux collections logiques et nettoyage des builds orphelins. Recall workflow encore principalement SQL; suppression multi-store sans journal compensatoire. |
...
| Design blueprint | IMPLEMENTED_LIMITED | `compose_design_blueprint`, `core/contracts/design_blueprint.py`, `design_blueprint.json`, deux rapports de couverture | Intents génériques, contraintes et spécialistes déterministes routés; preuve `RequirementSpec -> Blueprint -> SceneSpec` liée au certificat 1.1. Pas encore de candidats concurrents choisis par LLM, de connecteurs catalogués ni d’asset scoring multi-candidat. |
| Asset inventory | IMPLEMENTED | `/assets/inventory`, `core/services/asset_inventory.py` | 12 manifests/GLB, 10 profils éligibles, 4 GLB autorisés en import exact par hash et contrôles géométriques, 2 références seules; statut `qualified_mixed_catalog`. |
...
| Asset adaptation profiles | IMPLEMENTED_LIMITED | `/assets/adaptation-capabilities`, `/designs/{id}/adaptation-capabilities`, `assets/capabilities` | Catalogue typé et borné; tower paramétrique, layout secteur, offsets RRU et transforms accessoires opérationnels. Profils panel/RRU multi-pièces résolus par LOD dans `SceneSpec`; pas de retopologie GLB opaque ni matériau arbitraire. |
| Blender generation | IMPLEMENTED_LIMITED | `core/services/blender_runner.py`, `apps/blender_worker` | Réel si Blender trouvé; factory-startup, staging unique par tentative, copie immuable du bundle worker réellement exécutée et build lock hashé; fallback Blender refusé par défaut côté qualité. |
...
| Geometry QA | IMPLEMENTED_LIMITED | `core/qa/glb_geometry_validator.py`, `core/qa/mesh_qa.py`, `core/qa/glb_inspector.py` | `mesh_level_spatial_basic` quand transforms et bounds GLB sont complets, avec screening AABB; les profils panel/RRU internes exigent aussi leurs sous-pièces déclarées. Pas de collision triangle/BVH, RF ou vendor-grade. |
...
| Completion certificate | IMPLEMENTED | `core/validation/completion_certificate.py`, `core/services/scene_versioning.py`, `WorkflowService._enforce_completion_proof` | `completed` exige Blender réel, gates/QA valides et hashes GLB/preview/metadata/build lock. Les nouveaux certificats 1.1 lient aussi le hash du `DesignBlueprint`, ses couvertures, les rapports critiques persistés et la `SceneVersion.scene`; les résultats historiques incomplets sont mis en quarantaine. Ce n'est pas une signature externe. |
...
| Versioning / rollback | IMPLEMENTED | `core/services/scene_versioning.py` | Activation atomique par `active_design.json` après revalidation des hashes, de la scène versionnée et des rapports critiques 1.1. Les projections root/event best-effort ne peuvent plus déclasser un commit canonique. |
...
| Frontend product rework | IMPLEMENTED_LIMITED | `apps/frontend`; 117 tests Vitest + typecheck/build; 2026-07-29 desktop/mobile and headless-fallback smokes on real workflow `wf_a6660b81b929` | Chat-first/3D-first baseline; contradiction d'entrée bloquée, fallback/transport humanisés, fidélité géométrique visible, fondation documentaire corrigeable. Full acceptance exige encore un smoke visuel enregistré de chaque mutation produit. |
...
| `select_assets` / `asset_fallback_handler` | LangGraph node + asset registry | `AssetRegistry`, manifests | Seuls les manifests qualifiés sont sélectionnables; le mode et la raison sont inscrits dans `SceneSpec`, l'import exact vérifie le SHA-256, et tout fallback reste visible. |
...
| `compose_design_blueprint` | LangGraph node + routed deterministic specialists | `BlueprintComposer`, `design_blueprint.json` | Composition typée et hashée; pas encore superviseur autonome ni création libre de familles. |
